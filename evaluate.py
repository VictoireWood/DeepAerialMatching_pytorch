...
# USAGE:
# python3 evaluate_self.py MODE FOLDER_NAME DATAPATH MODEL_PATH VGG_MODEL_PATH --TEST_DATA_SAVE_PATH

# TRAIN:
# python3 evaluate_self.py train woodbridge ../sat_data/ trained_model_output.pth ../models/vgg16_model.pth

# TEST:x
# python3 evaluate_self.py test woodbridge ../sat_data/ ../models/conv_02_17_18_1833.pth ../models/vgg16_model.pth -t test_out.txt
...
def random_param_generator():
    ...
    if USE_CUDA:
        p_gt = Variable(torch.Tensor([scale * cos(rad_ang) - 1,
                                      -scale * sin(rad_ang),
                                      translation_x,
                                      scale * sin(rad_ang),
                                      scale * cos(rad_ang) - 1,
...
def static_data_generator(batch_size):
    ...
    for i in range(batch_size):
        ...
        img_seg_pad_w, _ = dlk.warp_hmg(img_seg_pad.unsqueeze(0),
                                        dlk.H_to_param(inv_func.forward(dlk.param_to_H(p_gt))))
...
def corner_loss(p, p_gt):
    ...
    if USE_CUDA:
        corners = Variable(
            torch.Tensor([[-training_sz_pad / 2, training_sz_pad / 2, training_sz_pad / 2, -training_sz_pad / 2],
...
def test():
    if USE_CUDA:
        dlk_vgg16 = dlk.DeepLK(dlk.vgg16Conv(BACKBONE_MODEL_PATH)).cuda()
        dlk_trained = dlk.DeepLK(dlk.custom_net(MODEL_PATH)).cuda()
    else:
        dlk_vgg16 = dlk.DeepLK(dlk.vgg16Conv(BACKBONE_MODEL_PATH))
    ...
    print('test batch size: ', testbatch_sz, ' number of test round: ', test_rounds_num, ' rounds per pair: ',
          rounds_per_pair)
...
def train():
    if USE_CUDA:
        dlk_net = dlk.DeepLK(dlk.ResNet18Conv(BACKBONE_MODEL_PATH)).cuda()
    else:
        dlk_net = dlk.DeepLK(dlk.ResNet18Conv(BACKBONE_MODEL_PATH))

    optimizer = optim.Adam(filter(lambda p: p.requires_grad, dlk_net.conv_func.parameters()), lr = 0.00001)
    ...
    print('BACKBONE MODEL PATH', BACKBONE_MODEL_PATH)
    print('USE CUDA: ', USE_CUDA)
    print('min_scale: ', min_scale)
    print('max_scale: ', max_scale)
    print('angle_range: ', angle_range)
    ...
    for epoch_idx in range(train_epoch):
        if USE_CUDA:
            valid_img_batch = Variable(
                torch.zeros(valid_batch_sz * valid_num_generator, 3, training_sz, training_sz)).cuda()
            valid_template_batch = Variable(
                torch.zeros(valid_batch_sz * valid_num_generator, 3, training_sz, training_sz)).cuda()
            ...
            valid_template_batch = Variable(
                torch.zeros(valid_batch_sz * valid_num_generator, 3, training_sz, training_sz))
        ...
        for i in range(valid_num_generator):
            ...
            valid_img_batch[i * valid_batch_sz: i * valid_batch_sz + valid_batch_sz, :, :, :], valid_template_batch[
                                                                                               i * valid_batch_sz: i * valid_batch_sz + valid_batch_sz,
                                                                                               :, :,
                                                                                               :], valid_param_batch[
                                                                                                   i * valid_batch_sz: i * valid_batch_sz + valid_batch_sz,
                                                                                                   :,
        ...
        for i in range(num_minibatch - 4):
            start = time.time()
            optimizer.zero_grad()

            img_batch_unnorm = img_train_data[i:i + 4, :, :, :]
            template_batch_unnorm = template_train_data[i:i + 4, :, :, :]
            training_param_batch = param_train_data[i:i + 4, :, :]
            ...
            dlk_param_batch, _ = dlk_net(training_img_batch, training_template_batch, tol = 1e-3, max_itr = 120,
                                         conv_flag = 1)
            ...
            if i % 10 == 0:
                dlk_valid_param_batch, _ = dlk_net(valid_img_batch, valid_template_batch, tol = 1e-3, max_itr = 120,
                                                   conv_flag = 1)
                ...
                if (epoch_idx == 0) or (float(valid_loss) < float(best_valid_loss)):
        ...
        del img_train_data, template_train_data, param_train_data
